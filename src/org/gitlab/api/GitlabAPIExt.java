...
import org.gitlab.api.http.GitlabHTTPRequestorExt;
...
public class GitlabAPIExt {
    ...
    private GitlabAPIExt(String hostUrl, String apiToken, TokenType tokenType, AuthMethod method) {
    ...
    public static GitlabSession connect(String hostUrl, String username, String password) throws IOException {
        ...
        GitlabAPIExt api = connect(hostUrl, null, null, null);
        return api.dispatch().with("login", username).with("password", password)
                .to(tailUrl, GitlabSession.class);
    }

    public static GitlabAPIExt connect(String hostUrl, String apiToken) {
        return new GitlabAPIExt(hostUrl, apiToken, TokenType.PRIVATE_TOKEN, AuthMethod.HEADER);
    }

    public static GitlabAPIExt connect(String hostUrl, String apiToken, TokenType tokenType) {
        return new GitlabAPIExt(hostUrl, apiToken, tokenType, AuthMethod.HEADER);
    }

    public static GitlabAPIExt connect(String hostUrl, String apiToken, TokenType tokenType, AuthMethod method) {
        return new GitlabAPIExt(hostUrl, apiToken, tokenType, method);
    }

    public GitlabAPIExt ignoreCertificateErrors(boolean ignoreCertificateErrors) {
    ...
    public GitlabAPIExt setRequestTimeout(int requestTimeout) {
        this.requestTimeout = requestTimeout;
        return this;
    }

    public GitlabHTTPRequestorExt retrieve() {
        return new GitlabHTTPRequestorExt(this).authenticate(apiToken, tokenType, authMethod);
    }

    public GitlabHTTPRequestorExt dispatch() {
        return new GitlabHTTPRequestorExt(this).authenticate(apiToken, tokenType, authMethod).method("POST");
    }
    ...
    public List<GitlabUser> getUsers(String username) throws IOException {
        String tailUrl = GitlabUser.URL;
        GitlabUser[] gitlabUsers = retrieve().with("username", username).to(tailUrl, GitlabUser[].class);
    ...
    public List<GitlabProject> getProjects(String projectName) throws IOException {
        String tailUrl = GitlabProject.URL + "/all";
        GitlabProject[]  gitlabProjects = retrieve().with("search", projectName).to(tailUrl, GitlabProject[].class);
    ...
    public GitlabMergeRequest acceptMergeRequest(GitlabProject project, Integer mergeRequestId, String mergeCommitMessage) throws IOException {
        ...
        GitlabHTTPRequestorExt requestor = retrieve().method("PUT");
    ...
    public GitlabRepositoryFile getRepositoryFile(GitlabProject project, String filePath, String refName) throws IOException {
        Query query = new Query()
                .append("file_path", filePath)
                .append("ref", refName);

        String tailUrl = GitlabProject.URL + "/" + project.getId() + "/repository" + GitlabRepositoryFile.URL + query.toString();
        GitlabRepositoryFile file = retrieve().to(tailUrl, GitlabRepositoryFile.class);
        return file;
	}
    /**
    ...
    public GitlabIssue createIssue(int projectId, int assigneeId, int milestoneId, String labels,
        ...
        GitlabHTTPRequestorExt requestor = dispatch();
    ...
    private void applyIssue(GitlabHTTPRequestorExt requestor, int projectId,
    ...
    public GitlabMilestone createMilestone(
        ...
        GitlabHTTPRequestorExt requestor = dispatch().with("title", title);
        if (description != null) {
    ...
    public GitlabProjectMember getProjectMember(Integer projectId, Integer userId) throws IOException {
    	String tailUrl = GitlabProject.URL + "/" + projectId + GitlabProjectMember.URL  + "/" + userId;
        return retrieve().to(tailUrl, GitlabProjectMember.class);
    }
    ...
    public GitlabCommit createCommit(Integer projectId, String branchName, String commitMessage, JSONArray actions) throws IOException {
        
//    	Query query = new Query()
//    			.append("id", projectId.toString())
//    			.append("branch_name", branchName)
//    			.append("commit_message", commitMessage)
//    			.append("actions", actions);
    	String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) + "/repository/commits";

    	return dispatch().with("id", projectId.toString()).with("branch_name", branchName).with("commit_message", commitMessage).with("actions", actions).to(tailUrl, GitlabCommit.class);
    }
    /**
